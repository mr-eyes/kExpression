...
@click.option('-s','--chunk-size', 'chunk_size', required=False, type=click.INT, default = 100, show_default=True, help="chunk size, how many reads to process each iteration?")
...
def batchQuery(ctx, index_prefix, chunk_size, fasta_file, kSize):
    ...
    BQ = Batch_Query(logger_obj = ctx.obj, index_prefix = index_prefix, fasta_file = fasta_file, chunk_size = chunk_size, kSize = kSize)
    BQ.process()